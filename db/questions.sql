...
SELECT town_health_records.town, town_health_records.population_greater_than_65_2005
  FROM town_health_records
ORDER BY town_health_records.population_greater_than_65_2005 DESC LIMIT 3;
...
SELECT town, percent_teen_births_2005_to_2008
  FROM town_health_records
  WHERE NOT town
    IN ('Boston', 'Becket', 'Beverly')
    AND percent_teen_births_2005_to_2008 IS NOT NULL
...
SELECT town, infant_mortality_rate_per_thousand_2005_to_2008
  FROM town_health_records
  WHERE town != 'Boston'
    AND infant_mortality_rate_per_thousand_2005_to_2008 IS NOT NULL
...
SELECT town, per_capita_income_2000, persons_below_poverty_2000
  FROM
    (SELECT town, per_capita_income_2000, persons_below_poverty_2000
      FROM town_health_records
    ORDER BY per_capita_income_2000 DESC LIMIT 5)
    AS foo
